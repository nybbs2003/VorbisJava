 ...
 * https://wiki.xiph.org/index.php/MIME_Types_and_File_Extensions but we try to
 * use more specific ones, as given by the Tika mimetypes xml file.
 */
public class OggDetector implements Detector {
	...
	public MediaType detect(InputStream input, Metadata metadata) throws IOException {
		...
		input.mark(4);
		try {
			if (input.read() != (byte) 'O' || input.read() != (byte) 'g' || input.read() != (byte) 'g' || input.read() != (byte) 'S') {
				return MediaType.OCTET_STREAM;
			}
		} finally {
			input.reset();
		...
		if (tis != null) {
			...
			// Open the Ogg file - underlying stream stays open as detecting
			// only
			...
			Map<Integer, List<SkeletonPacket>> skeletonStreams = new HashMap<Integer, List<SkeletonPacket>>();
