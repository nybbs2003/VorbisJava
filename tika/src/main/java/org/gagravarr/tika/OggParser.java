 ...
 * 
 * We provide a detector which should help specialise the more common kinds of
 * Ogg files, based on their streams, to their appropriate types. This just
 * handles the rest, as best we can.
 */
public class OggParser extends AbstractParser {
	...
	private static List<MediaType> TYPES = Arrays.asList(new MediaType[] {
			// General ones, which didn't fit a specific pattern (eg
			// 2*Opus+2*Vorbis,
			...
			DAALA_VIDEO, DIRAC_VIDEO, OGM_VIDEO, OGG_UVS, OGG_YUV, OGG_RGB, OGG_PCM, KATE });
	...
	public void parse(InputStream stream, ContentHandler handler, Metadata metadata, ParseContext context) throws IOException, TikaException, SAXException {
		...
		Map<OggStreamType, Integer> streams = new HashMap<OggStreamType, Integer>();
		Map<OggStreamType.Kind, Integer> streamKinds = new HashMap<OggStreamType.Kind, Integer>();
		...
		// If we found a skeleton stream, report the relations between the
		// streams
		// TODO
